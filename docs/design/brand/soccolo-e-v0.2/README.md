# Soccolo Concept E reference-aligned refinement — v0.2

Status: **provisional review candidate; not an approved launch identity**.

This directory contains the revised Section 4 asset and provenance bundle requested by [SP-076 brand assets and ownership decision](../../../decisions/SP-076-brand-assets-and-ownership.md). The mark is a deliberate code-native reconstruction of the founder-selected Double Touch Wordmark direction. Version 0.2 restores the reference's defining inward-facing mint `c · c` unit within `Soccolo`; it is redrawn from controlled geometry rather than traced from the raster reference.
...
Reference-aligned vector revision:

![Soccolo Concept E v0.2 review board](soccolo-review-board.png)
...
Founder direction selection does not approve these files for launch. Approval requires review of the exact v0.2 files/checksums, resolution of the similarity-review warnings—especially the high-risk standalone inward-facing `C·C` device—and a dated approve/revise/stop decision. The selected unit may continue inside the full wordmark for internal review, but the app/device symbol should not be treated as launch-ready. Trade-mark/legal clearance remains separate.
