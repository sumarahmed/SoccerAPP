# Soccolo identity usage specification — Concept E v0.2

## Identity and status

- Working name: `Soccolo`.
- Working theme: `Train, Play, Grow`.
- Brand ID: `working-soccolo-e-v0.2`.
...
The wordmark and symbol are repository geometry built from paths and circles. Matching the founder-supplied selected reference, the two `c` forms face inward toward a shared touch point. The complete unit uses one accent colour: deep green on light surfaces and mint on dark surfaces. It is reconstructed from symmetric editable paths rather than traced pixels.
...
- represent v0.2 as a cleared, registered or approved launch asset.
