...
Print artwork lives in `07-print/`, separately from the screen files, because
...
| Folder | What it is |
|---|---|
| `cmyk-for-print/` | CMYK vector PDF, FOGRA39L Coated output intent embedded. Use these on press. |
| `rgb-for-office/` | sRGB vector PDF, for documents, slides and email. |
| `soccolo-print-reference.pdf` | Two pages: colour build, gamut, sizes, stock notes, file index. Give this to the printer. |
| `print-index.json` | The same data, machine-readable. |

Every page is trimmed to the artwork plus its clear space, so placing the file
edge to edge in a layout still respects the spacing rule. There is no bleed,
because there is no background: the mark sits on whatever ground you place it on.

No fonts are embedded, nothing is transparent, and the fills are non-zero
winding, so counters are real holes rather than white shapes laid on top. The
files are clean CMYK vector PDFs with an embedded output intent; they are not
claimed as certified PDF/X.

### The colour build

Converted from sRGB through **FOGRA39L Coated at relative colorimetric intent**,
measured rather than calculated by hand. The full table is on the reference sheet
and in `print-index.json`. The two values that matter most:

| Token | CMYK | Round-trip error |
|---|---|---|
| `ink-800` brand navy | 98 / 49 / 15 / 92 | 2.5 dE |
| `pitch-700` deep green | 95 / 28 / 90 / 53 | 0.5 dE |

### The mint does not print

**`pitch-300` lands 30.1 dE from its screen value in four-colour process.** Past
about 3 dE the difference is visible; 30 is a different colour. The whole bright
end of the green ramp is outside the CMYK gamut: `pitch-400` at 20.3,
`pitch-200` at 24.5, `pitch-100` at 10.9.

So, for print:

1. **Use the deep green.** `pitch-700` converts at 0.5 dE, which is as close as
   process colour gets. It is already the accent on light grounds, so most print
   needs no change at all.
2. **Where the mint is genuinely required** (the mark reversed on navy, for
   instance) it needs a **fifth plate as a spot colour**. Have the printer match
   it against a physical guide and sign off a drawdown.
3. **No spot reference is given in this bundle**, because none has been verified
   against a physical sample. Do not let a guessed Pantone number be substituted
   into the specification without a drawdown to check it against.

Highest total area coverage in the palette is 280% (`pitch-900`), inside the
330% FOGRA39 limit, so no ink-limit problem arises.

### Stock

| Stock | What to do |
|---|---|
| Coated | As specified. The build is measured against this condition. |
| Uncoated | Ink spreads and the navy gains. Proof on the actual stock and expect to lighten the navy build. |
| Newsprint, low grade | Use the one-colour black file. A four-colour navy will fill in. |
| One colour | Use the black or white files, never a tint or screen of the navy. |
| Foil, emboss, screen print | Supply the one-colour black as artwork, and re-test the minimum sizes physically. |
...
04-export-treatment/  Fixed navy end frames, safe-area overlays, timing specification
05-specification/     This document, tokens, clear space, minimum size
06-provenance/  Rights record, similarity review, revision log
07-print/       CMYK and RGB vector PDFs, printer's reference
08-video/       Two-second intros, opt-in outros, exporter and media manifest
release-manifest.json  SHA-256 integrity record for the complete bundle
```
