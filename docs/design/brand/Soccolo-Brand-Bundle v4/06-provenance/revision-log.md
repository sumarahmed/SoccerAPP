...
## v4.0, same day: corrected recording-export package and founder acceptance

- V2 remains the accepted provenance, artwork and print baseline. V3 was
  reviewed separately and was not adopted as a release bundle.
- The acceptable v3 video idea was rebuilt into v4: fixed Soccolo plates for
  1.60 seconds, a 0.40-second dissolve to a held copy of source frame zero, and
  unobscured source motion and source audio beginning at export time 2.00.
- The reference exporter contains source media, never crops or upscales it by
  default, refuses overwrites, keeps the optional four-second outro disabled by
  default, and validates exact output duration and decoded frame count.
- Dry runs make no filesystem changes. Output media and its JSON sidecar are
  staged, validated and only then published. Source SHA-256 is checked before
  and after export.
- Eighteen derived video assets have a codec, dimensions, duration, frame count
  and SHA-256 manifest. The complete bundle has a deterministic SHA-256 release
  manifest.
- No v2 rights, account, font, licence, trademark-risk or founder-acceptance
  statement was broadened. Syed Ahmed accepted v4, the composition and the
  export-permission boundary recorded in `ACT-SP-080-01` on 14 September 2026;
  no independent privacy, device, production or legal-clearance review is
  claimed.
