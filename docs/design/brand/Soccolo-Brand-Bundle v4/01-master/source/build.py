...
VIDEO_SOURCE = os.path.join(BUNDLE_SOURCE, "08-video")
if VIDEO_SOURCE not in sys.path:
    sys.path.insert(0, VIDEO_SOURCE)
import video as VID
import video_manifest as VIDEO_MANIFEST
import release_manifest as RELEASE_MANIFEST
ROOT = None
WORK = None
UNIT = L.UNIT
...
def copy_source():
    for f in ["geom.py", "system.py", "fontmark.py", "marks.py", "lockups.py",
              ...
              "printcolour.py", "sheets.py", "release_manifest.py",
              "requirements.txt"]:
...
def copy_authored_documents():
    ...
    copies = [
        ...
        (os.path.join("07-print", "soccolo-print-reference.pdf"),
         os.path.join("07-print", "soccolo-print-reference.pdf")),
        (os.path.join("08-video", "video.py"),
         os.path.join("08-video", "video.py")),
        (os.path.join("08-video", "video_manifest.py"),
         os.path.join("08-video", "video_manifest.py")),
        (os.path.join("08-video", "soccolo-export.py"),
         os.path.join("08-video", "soccolo-export.py")),
        (os.path.join("08-video", "video-export-specification.md"),
         os.path.join("08-video", "video-export-specification.md")),
    ]
...
def main(argv=None):
    ...
    for f in ["clear-space.svg", "minimum-size.png", "colour-tokens.json",
        ...
        if os.path.exists(fp):
            os.remove(fp)
    for sub in ["intro", "outro", "mark"]:
        shutil.rmtree(os.path.join(ROOT, "08-video", sub), ignore_errors=True)
    manifest_path = os.path.join(ROOT, "08-video", "manifest.json")
    ...
    try:
        ...
        video_files = VID.build_assets(os.path.join(ROOT, "08-video"), fps=30)
        original_video_root = VIDEO_MANIFEST.ROOT
        original_manifest = VIDEO_MANIFEST.MANIFEST
        try:
            VIDEO_MANIFEST.ROOT = os.path.join(ROOT, "08-video")
            VIDEO_MANIFEST.MANIFEST = os.path.join(VIDEO_MANIFEST.ROOT, "manifest.json")
            VIDEO_MANIFEST.main(["--write"])
        finally:
            VIDEO_MANIFEST.ROOT = original_video_root
            VIDEO_MANIFEST.MANIFEST = original_manifest
        m = measure()
        with open(path("05-specification", "measurements.json"), "w") as f:
            json.dump(m, f, indent=2)
        RELEASE_MANIFEST.main(["--write", ROOT])
        print("output:", ROOT)
