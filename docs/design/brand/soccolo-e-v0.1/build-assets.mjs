...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title desc" ${extra}>
  ...
  <metadata>Soccolo Concept E controlled refinement v0.1; generated from repository source geometry on 2026-09-14; no stock icon, traced image or third-party wordmark font.</metadata>
${body}
</svg>
`;
...
function symbol(primary, accent, dot = primary) {
  return `<g fill="none" stroke-linecap="round" stroke-linejoin="round">
    <path d="M88 27 C72 11 40 13 24 35 C7 58 19 91 48 99 C65 104 82 98 93 84" stroke="${primary}" stroke-width="18"/>
    <circle cx="111" cy="58" r="8.5" fill="${dot}" stroke="none"/>
    <path d="M198 22 C182 6 150 8 134 30 C117 53 129 86 158 94 C175 99 192 93 203 79" stroke="${accent}" stroke-width="18"/>
  </g>`;
}
...
function wordmark(primary, accent, dot = primary) {
  return `<g fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="18">
    ...
    <circle cx="233" cy="58" r="5.5" fill="${dot}" stroke="none"/>
    <path d="M304 33 C290 18 265 20 251 37 C238 54 243 80 260 91 C275 100 294 95 305 81" stroke="${accent}"/>
...
function stackedLockup(background, primary, accent, dot, includeBackground = true) {
  ...
  <g transform="translate(20 244)">${wordmark(primary, accent, dot)}</g>
  ${vectorText('Train, Play, Grow', primary, 135, 365, 1.15)}`;
}

const outputs = new Map([
  ['soccolo-symbol-primary.svg', xml(220, 120, 'Soccolo double-touch symbol', 'Two sequential custom C-shaped training touches and one contact point in navy and green.', symbol(colours.navy, colours.green))],
  ['soccolo-symbol-reversed.svg', xml(220, 120, 'Soccolo double-touch reversed symbol', 'Two sequential custom C-shaped training touches and one contact point in white and mint.', symbol(colours.white, colours.mint, colours.white))],
  ['soccolo-wordmark-primary.svg', xml(530, 120, 'Soccolo custom wordmark', 'Custom geometric Soccolo wordmark with differentiated double-c touch construction.', wordmark(colours.navy, colours.green))],
  ['soccolo-wordmark-reversed.svg', xml(530, 120, 'Soccolo reversed custom wordmark', 'Custom geometric Soccolo wordmark in white and mint for dark surfaces.', wordmark(colours.white, colours.mint, colours.white))],
  ...
  ['soccolo-app-icon-1024.svg', xml(1024, 1024, 'Soccolo app icon', 'Full-bleed navy square with the white and mint double-touch symbol in the central safe area.', `<rect width="1024" height="1024" fill="${colours.navy}"/><g transform="translate(103 287) scale(3.72)">${symbol(colours.white, colours.mint, colours.white)}</g>`)],
  ['soccolo-android-adaptive-foreground.svg', xml(432, 432, 'Soccolo Android adaptive foreground', 'Transparent adaptive-icon foreground with the white and mint double-touch symbol inside the 66 by 66 percent safe zone.', `<g transform="translate(62 134) scale(1.4)">${symbol(colours.white, colours.mint, colours.white)}</g>`)],
...
const reviewBoard = xml(1600, 1040, 'Soccolo Concept E v0.1 asset review board', 'Review board showing the refined custom wordmark, symbol, light, dark, monochrome, app icon and small-size treatments.', `<rect width="1600" height="1040" fill="${colours.surfaceLight}"/>
  <text x="70" y="72" fill="${colours.navy}" font-family="Arial, sans-serif" font-size="34" font-weight="700">SOCCOLO — CONCEPT E CONTROLLED REFINEMENT v0.1</text>
...
writeFileSync(join(outputDir, 'brand-tokens.json'), `${JSON.stringify({
  schemaVersion: 1,
  brandId: 'working-soccolo-e-v0.1',
