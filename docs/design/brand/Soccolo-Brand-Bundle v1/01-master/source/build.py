...
Running this file regenerates every deliverable in the bundle from the
construction itself. Nothing in the output is hand-edited, which is what makes
the mark reproducible and the provenance record verifiable.
"""
import argparse
import json
import os
...
# Keep generated/copyable source trees free of machine-specific bytecode files.
...
def _font(size, bold=False):
    ...
    # Use the supplied face so proof rendering is deterministic on every OS.
...
def copy_source():
    for f in ["geom.py", "system.py", "fontmark.py", "marks.py", "lockups.py",
              "palette.py", "svgdoc.py", "build.py", "requirements.txt"]:
        shutil.copy2(os.path.join(SOURCE_DIR, f),
                     path("01-master", "source", f))
...
def copy_authored_documents():
    """Copy records and guidance that are deliberately not generated."""
    copies = [
        ("README.md", "README.md"),
        ...
        (os.path.join("04-export-treatment", "soccolo-export-timing-specification.pdf"),
         os.path.join("04-export-treatment", "soccolo-export-timing-specification.pdf")),
        (os.path.join("02-logo", "print", "soccolo-print-reference.pdf"),
         os.path.join("02-logo", "print", "soccolo-print-reference.pdf")),
...
def main(argv=None):
    ...
    # the source bundle are copied after generation.
    for sub in ["01-master", "02-logo", "03-app", "04-export-treatment"]:
    ...
    try:
        ...
        print("output:", ROOT)
        print("logo files:", len(mani), "| contrast audit passes:", ok,
              "| proof sheet:", sz)
        for k, v in m.items():
            print(f"  {k:24} {v}")
    finally:
