...
def _bounds_from(paths):
    pts = [q for d in paths for q in FM._path_points(d)]
    xs, ys = [q[0] for q in pts], [q[1] for q in pts]
    # emitted paths are y-down; bounds are reported y-up
    return (min(xs), -max(ys), max(xs), -min(ys))


def wordmark_plain():
    paths, _ = FM.typeset(SY.FACE, "Soccolo", cap=SY.CAP)
    b = _bounds_from(paths)
    return Mark("wordmark-plain", paths, b, {"advance": b[2] - b[0]})
...
def wordmark_accent():
    ...
    base = head + tail
    b = _bounds_from(base + accent)
    return Mark("wordmark-accent", base + accent, b,
...
def tagline(tracking=None):
    ...
    paths, _ = FM.typeset(SY.FACE, "Train, Play, Grow", cap=SY.CAP, tracking=tr)
    b = _bounds_from(paths)
    return Mark("tagline", paths, b, {"advance": b[2] - b[0]})
