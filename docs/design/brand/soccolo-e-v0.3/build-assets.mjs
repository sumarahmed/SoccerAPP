...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title desc" ${extra}>
  ...
  <metadata>Soccolo Concept E controlled refinement v0.3; generated from repository source geometry on 2026-09-14; no stock icon, traced image or third-party wordmark font.</metadata>
${body}
</svg>
`;
...
function symbol(primary, accent, dot = primary) {
  return `<g fill="none" stroke-linecap="round" stroke-linejoin="round">
    ...
    <circle cx="115" cy="58" r="12" fill="${accent}" stroke="none"/>
    <path d="M218 22 C202 6 170 8 154 30 C137 53 149 86 178 94 C195 99 212 93 223 79" stroke="${accent}" stroke-width="18"/>
  </g>`;
}
...
function wordmark(primary, accent, dot = primary) {
  return `<g fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="18">
    ...
    <circle cx="238" cy="58" r="10.5" fill="${accent}" stroke="none"/>
    <path d="M329 33 C315 18 290 20 276 37 C263 54 268 80 285 91 C300 100 319 95 330 81" stroke="${accent}"/>
    <circle cx="355" cy="59" r="31" stroke="${primary}"/>
    <path d="M411 17 L411 79 C411 94 423 100 437 89" stroke="${primary}"/>
    <circle cx="486" cy="59" r="31" stroke="${primary}"/>
  </g>`;
}
...
const outputs = new Map([
  ['soccolo-symbol-primary.svg', xml(240, 120, 'Soccolo forward double-touch symbol', 'Two custom left-to-right C-shaped training touches and one enlarged contact point in green.', symbol(colours.navy, colours.green))],
  ['soccolo-symbol-reversed.svg', xml(240, 120, 'Soccolo forward double-touch reversed symbol', 'Two custom left-to-right C-shaped training touches and one enlarged contact point in mint.', symbol(colours.white, colours.mint, colours.white))],
  ['soccolo-wordmark-primary.svg', xml(540, 120, 'Soccolo custom wordmark', 'Custom geometric Soccolo wordmark with overlapping o and c forms, two left-to-right c forms and an enlarged touch point.', wordmark(colours.navy, colours.green))],
  ['soccolo-wordmark-reversed.svg', xml(540, 120, 'Soccolo reversed custom wordmark', 'Custom geometric Soccolo wordmark in white and mint with overlapping o and c forms, two left-to-right c forms and an enlarged touch point.', wordmark(colours.white, colours.mint, colours.white))],
  ...
  ['soccolo-app-icon-1024.svg', xml(1024, 1024, 'Soccolo app icon', 'Full-bleed navy square with the mint forward double-touch symbol in the central safe area.', `<rect width="1024" height="1024" fill="${colours.navy}"/><g transform="translate(103 287) scale(3.72)">${symbol(colours.white, colours.mint, colours.white)}</g>`)],
  ['soccolo-android-adaptive-foreground.svg', xml(432, 432, 'Soccolo Android adaptive foreground', 'Transparent adaptive-icon foreground with the mint forward double-touch symbol inside the 66 by 66 percent safe zone.', `<g transform="translate(62 134) scale(1.4)">${symbol(colours.white, colours.mint, colours.white)}</g>`)],
  ...
  ['soccolo-favicon.svg', xml(240, 120, 'Soccolo favicon source', 'Transparent one-accent Soccolo double-touch symbol source for favicon rasterization.', symbol(colours.navy, colours.green))],
...
const reviewBoard = xml(1600, 1040, 'Soccolo Concept E v0.3 asset review board', 'Review board showing overlapping o-c joins, two left-to-right c forms, the enlarged touch point, light, dark, monochrome, app icon and small-size treatments.', `<rect width="1600" height="1040" fill="${colours.surfaceLight}"/>
  <text x="70" y="72" fill="${colours.navy}" font-family="Arial, sans-serif" font-size="34" font-weight="700">SOCCOLO — CONCEPT E FORWARD DOUBLE-TOUCH REFINEMENT v0.3</text>
...
writeFileSync(join(outputDir, 'brand-tokens.json'), `${JSON.stringify({
  schemaVersion: 1,
  brandId: 'working-soccolo-e-v0.3',
